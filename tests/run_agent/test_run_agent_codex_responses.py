...
def test_build_api_kwargs_codex_omits_absent_tools(monkeypatch):
    agent = _build_agent(monkeypatch)
    agent.tools = []
...
def test_run_codex_stream_falls_back_when_completed_output_is_null(monkeypatch):
    agent = _build_agent(monkeypatch)
    calls = {"stream": 0, "create": 0}
    create_stream = _FakeCreateStream(
        [
...
def test_run_codex_stream_recovers_when_sdk_parser_sees_null_output_after_events(
...
def test_run_codex_stream_recovers_when_get_final_response_sees_null_output(monkeypatch):
    agent = _build_agent(monkeypatch)
